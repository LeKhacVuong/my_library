...
typedef struct sm_fw_signature{
    ...
    uint32_t m_size;
    uint16_t m_crc;
    int32_t m_data_address_offset;
    uint8_t m_is_valid;
}sm_fw_signature_t;

typedef struct{
    uint32_t        m_new_fw_flag;
    uint32_t        m_size;
    uint32_t        m_addr;
    uint8_t         m_is_valid;
    uint8_t         m_version[3];
    uint16_t        m_crc;
}sm_boot_2_fw_setting_t;
...
static inline int32_t sm_fw_signature_reset(sm_fw_signature_t* _this){
    if(!_this){
        return -1;
    }
    memset(_this->m_version, 0, SM_FW_VERSION_LENGTH);
    _this->m_size = 0;
    _this->m_addr = 0;
    _this->m_crc = 0;
    _this->m_is_valid = 0;
