...
struct sm_fw{
    const sm_fw_storage_opt_t* m_mem_opt;
...
sm_fw_t* sm_fw_create(const sm_fw_storage_opt_t* _mem_opt){
    ...
    obj = (sm_fw_t*)mem_alloc(sizeof(sm_fw_t));
    if(!obj){
        return NULL;
    }
...
int32_t sm_fw_is_valid(sm_fw_t* _self){
    ...
    if(_self->m_signature.m_is_firt_time_update == 1){
        LOG_INF(TAG, "Firmware is never been ota, set firmware is valid!!!");
    	return 1;
    }

    uint16_t crc_val = CRC_CalculateCRC16(_self->m_mem_opt->read(_self->m_signature.m_addr), _self->m_signature.m_size);
...
int32_t sm_fw_format(sm_fw_t* _self){
    ...
    return _self->m_mem_opt->erase_full();
}
...
int32_t sm_fw_write_segment(sm_fw_t* _self, const sm_segment_t* _segment){
    ...
    return _self->m_mem_opt->write_segment(addr, code, size);
}
