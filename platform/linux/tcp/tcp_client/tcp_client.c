...
int tcp_client_connect(tcp_client_t* self, const char* host_name, int port){
    ...
    if(self){
        ...
        struct hostent *host;
        struct addrinfo hints = {0, AF_UNSPEC, SOCK_STREAM, IPPROTO_TCP, 0, NULL, NULL, NULL}; //SOCK_STREAM = TCP, SOCK_DGRAM = UDP
        ...
        if(!rc) {
            int fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP); //AF_INET6 for IPv6
            if (fd < 0) {
                return -1;
            }
